...
export function Nav() {
  ...
  return <nav className="flex flex-wrap justify-between items-center">
    ...
    {/* Burger button */}
    <button className="lg:hidden p-2 focus:ring-2 focus:ring-gray-200 rounded-lg hover:bg-gray-100"
    ...
    <div className={`${isMobileMenuShown === false && "hidden"} w-full lg:block lg:w-auto`}>
      <ul className="lg:space-x-8 flex flex-col lg:flex-row bg-gray-50 text-lg border border-gray-100 rounded-lg p-4 lg:bg-transparent lg:border-none">
        {ROUTES.map((route, i) => {
          return <li className={`rounded cursor-pointer py-2 px-3 ${i === 0 ? "bg-blue-500 text-white lg:bg-transparent lg:text-blue-500" : "hover:bg-gray-100"}`} key={route}>{route}</li>
        })}
      </ul>
    </div>
